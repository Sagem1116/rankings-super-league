...
function CoachProfilePage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={p.nome} description={`Nacionalidade: ${p.nac || "—"}`} />

      <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
        <Stat label="Campeão" value={p.campeao.length} detail={fmt(p.campeao)} />
        <Stat label="Promovido" value={p.promovido.length} detail={fmt(p.promovido)} />
        ...
        <Stat label="Despromovido" value={p.despromovido.length} detail={fmt(p.despromovido)} />
      </div>
